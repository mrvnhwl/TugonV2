...
function DifficultySelector() {
  const router = useNavigate(); // Initialize the useNavigate hook
            ...
            {(["Easy", "Average", "Hard"] as const).map((level) => (
              <div key={level} className="relative group">
                <Button
                  ...
                  onClick={() => {
                    if (level === "Easy") {
                      router("/eEvaluationPhase1"); // Navigate to Easy phase
                    } else if (level === "Average") {
                      router("/aEvaluationPhase1"); // Navigate to Average phase
                    } else if (level === "Hard") {
                      router("/hEvaluationPhase1"); // Navigate to Hard phase (update if needed)
                    }
                  }}
                >
                  {level}
