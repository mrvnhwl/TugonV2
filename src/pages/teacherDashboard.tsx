...
export default function TeacherDashboard() {
  ...
  const [analyticsTab, setAnalyticsTab] = useState<'class' | 'student'>('class');
              ...
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex p-1 rounded-xl bg-gray-100">
                  <button
                    ...
                    className={`px-3 py-1 text-xs font-semibold rounded-lg transition ${analyticsTab === 'class' ? 'bg-white shadow-sm' : 'text-gray-500'}`}
                    style={analyticsTab === 'class' ? { color: color.teal } : {}}
                  >
                    Class Overview
                  </button>
                  <button
                    onClick={() => setAnalyticsTab('student')}
                    className={`px-3 py-1 text-xs font-semibold rounded-lg transition ${analyticsTab === 'student' ? 'bg-white shadow-sm' : 'text-gray-500'}`}
                    style={analyticsTab === 'student' ? { color: color.teal } : {}}
                  >
                    Student Detail
                  </button>
                </div>
                  ...
                  {studentProgress.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                      ...
                      <div className="lg:col-span-1 space-y-2 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
                        {pagedStudents.map((s) => (
                          <div
                            ...
                            className={`cursor-pointer rounded-xl p-3 transition flex items-center justify-between ${
                            ...
                            style={{
                              ...
                              background: selectedStudentId === s.student_id ? `${color.teal}10` : "#fff",
                              borderColor: selectedStudentId === s.student_id ? color.teal : color.mist,
                              ringColor: color.teal
                            }}
                          >
                            <div className="min-w-0">
                              <div className="font-semibold text-sm truncate" style={{ color: color.deep }}>
                                {s.student_name}
                              </div>
                            ...
                            <div className="text-xs font-bold" style={{ color: color.teal }}>
                        ...
                        <div className="pt-4 flex items-center justify-between">
                          <div className="text-xs" style={{ color: color.steel }}>
                            {pagedStudents.length} / {studentProgress.length}
                          </div>
                      ...
                      <div className="lg:col-span-2">
                        {selectedStudentId ? (
                          <StudentDetail studentId={selectedStudentId} allProgress={progressAll} />
                        ) : (
                          <div className="h-full flex flex-col items-center justify-center text-center p-8 rounded-2xl border-2 border-dashed" style={{ borderColor: color.mist }}>
                            <Users className="h-12 w-12 mb-4 opacity-20" style={{ color: color.teal }} />
...
function StudentDetail({
  ...
  const studentData = allProgress.filter((p) => p.user_id === studentId);
  if (!studentData.length) return null;

  const latest = studentData[0];
  const studentEmail = latest.user_email;
  const scores = [...studentData].reverse().map((p, i) => ({
    i,
    score: toPercent(p.score),
    date: safeDate(p.completed_at),
    quiz: latest.quizzes?.title ?? "Untitled",
  }));
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-2xl p-6 shadow-inner ring-1"
    ...
    >
      <div className="flex justify-between items-start mb-6">
        ...
        <div className="bg-teal-100 text-teal-700 px-3 py-1 rounded-full text-xs font-bold">
          {studentData.length} Quizzes Completed
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-4">
          <div className="p-4 rounded-xl bg-gray-50 border" style={{ borderColor: color.mist }}>
            <span className="text-xs block mb-1" style={{ color: color.steel }}>Latest Quiz</span>
            <span className="font-semibold" style={{ color: color.deep }}>
              {latest.quizzes?.title ?? "Untitled"}
            </span>
          </div>
          <div className="p-4 rounded-xl bg-gray-50 border" style={{ borderColor: color.mist }}>
            <span className="text-xs block mb-1" style={{ color: color.steel }}>Latest Score</span>
            <span className="text-2xl font-bold" style={{ color: color.teal }}>
              {toPercent(latest.score)}%
            </span>
          </div>
        </div>

        <div className="h-48">
          <h4 className="text-xs font-semibold mb-2" style={{ color: color.steel }}>
            Score Trend (Last {scores.length} Quizzes)
          </h4>
          <ResponsiveContainer width="100%" height="100%">
            <RLineChart data={scores}>
              <RGrid strokeDasharray="3 3" />
              <RXAxis dataKey="i" hide />
              <RYAxis domain={[0, 100]} tick={{ fontSize: 10 }} />
              <RTooltip formatter={(v: any) => [`${v}%`, "Score"]} />
              <RLine type="monotone" dataKey="score" stroke={color.teal} strokeWidth={2} dot={{ r: 3 }} />
            </RLineChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="mt-6">
        <h4 className="text-sm font-semibold mb-3" style={{ color: color.deep }}>
          Completion History
        </h4>
        <div className="max-h-40 overflow-y-auto custom-scrollbar space-y-2">
          {studentData.map((p, i) => (
            <div key={i} className="flex justify-between items-center p-2 rounded-lg text-xs border" style={{ borderColor: color.mist }}>
              <span className="truncate flex-1" style={{ color: color.deep }}>
                {p.quizzes?.title ?? "Untitled Quiz"}
              </span>
              <span className="mx-3 text-gray-400">{safeDate(p.completed_at)}</span>
              <span className="font-bold" style={{ color: color.teal }}>{toPercent(p.score)}%</span>
            </div>
          ))}
        </div>
      </div>
