...
function Game() {
  ...
  const loadQuestion = async (index: number) => {
    try {
      ...
      if (questionsData && questionsData[index]) {
        ...
        if (answersData) {
          // Map the answer steps to our Answer interface
          // We'll use a custom is_correct logic or assume the table has it
          // Since tugonsense_answer_steps might not have is_correct,
          // we might need to check the implementation or add it to the baseline.
  ...
  const handleAnswer = async (answer: Answer) => {
    ...
    if (answer.is_correct) {
      const timeBonus = Math.floor((timeLeft / (currentQuestion.time_limit || 30)) * (currentQuestion.points || 1000));
      try {
        await supabase
          .from('game_participants')
          .update({
              ...
              <div className="mb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {answers.map((answer) => (
                    ...
                    >
                      {answer.answer_variants?.[0] || 'No answer available'}
                    </button>
                  ))}
                </div>
              </div>
